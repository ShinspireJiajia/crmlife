  ...
  filterTabs.forEach(function (tab) {
    tab.addEventListener('click', function () {
      ...
      if (filterType === 'all') {
        ...
        if (eventList) eventList.style.display = 'none';
        if (registeredList) registeredList.style.display = '';
      } else if (filterType === 'foundation' || filterType === 'family') {
        // 依分類篩選：顯示活動列表，隱藏已報名列表
        if (eventList) eventList.style.display = '';
        if (registeredList) registeredList.style.display = 'none';
        // 只顯示對應分類的卡片
        eventCards.forEach(function (card) {
          var category = card.getAttribute('data-category');
          card.style.display = (category === filterType) ? '' : 'none';
        });
      }
    ...
    card.addEventListener('click', function () {
      ...
      if (eventId) {
        ...
        if (category === 'foundation') {
          window.location.href = 'crm-forest-foundation-detail.html?id=' + eventId;
        } else {
