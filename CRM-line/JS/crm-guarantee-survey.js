...
document.addEventListener('DOMContentLoaded', function () {
  ...
  bankCards.forEach(function (card) {
    card.addEventListener('click', function (e) {
      // 避免點擊連結或輸入框時觸發選擇
      if (e.target.closest('.bank-link') || e.target.closest('.other-bank-input') || e.target.closest('.btn-view-images')) return;
  ...
  // --- 銀行對保圖片查看器 ---
  function makeBankImgPlaceholder(label) {
    var bg = '#f0ede8';
    var fg = '#a99480';
    var svg = [
      '<svg xmlns="http://www.w3.org/2000/svg" width="280" height="190">',
      '<rect width="280" height="190" fill="' + bg + '"/>',
      '<path d="M80 130L112 88L144 130Z" fill="' + fg + '" fill-opacity="0.28"/>',
      '<path d="M118 130L148 98L178 130Z" fill="' + fg + '" fill-opacity="0.18"/>',
      '<circle cx="185" cy="72" r="14" fill="' + fg + '" fill-opacity="0.22"/>',
      '<text x="140" y="160" text-anchor="middle" font-family="sans-serif" font-size="11.5" fill="' + fg + '" letter-spacing="0.3">' + label + '</text>',
      '</svg>'
    ].join('');
  ...
  var bankImageData = {
    'cathay': [
      makeBankImgPlaceholder('國泰世華・台中分行外觀'),
      makeBankImgPlaceholder('理財中心・對保窗口'),
      makeBankImgPlaceholder('客服聯絡人名片'),
    ],
    'bot': [
      makeBankImgPlaceholder('台灣銀行・台中分行'),
      makeBankImgPlaceholder('對保窗口資訊'),
    ],
    'hua-nan': [
      makeBankImgPlaceholder('華南銀行・台中分行'),
    ],
  };
  ...
  var imgViewer = document.getElementById('bankImgViewer');
  var imgViewerTitle = document.getElementById('bankImgViewerTitle');
  var imgViewerGrid = document.getElementById('bankImgViewerGrid');
  var imgViewerClose = document.getElementById('bankImgViewerClose');
  var imgViewerBackdrop = document.getElementById('bankImgViewerBackdrop');

  var imgFullscreen = document.getElementById('bankImgFullscreen');
  var imgFsImg = document.getElementById('bankImgFsImg');
  var imgFsClose = document.getElementById('bankImgFsClose');
  var imgFsBackdrop = document.getElementById('bankImgFsBackdrop');
  var imgFsPrev = document.getElementById('bankImgFsPrev');
  var imgFsNext = document.getElementById('bankImgFsNext');
  var imgFsCounter = document.getElementById('bankImgFsCounter');

  var fsCurrentList = [];
  var fsCurrentIndex = 0;

  function openBankImgViewer(bankValue) {
    var images = bankImageData[bankValue];
    if (!images || images.length === 0) return;
    var name = bankNameMap[bankValue] || bankValue;

    imgViewerTitle.textContent = name + ' · 對保相關資料';
    imgViewerGrid.innerHTML = '';

    images.forEach(function(src, i) {
      var thumb = document.createElement('div');
      thumb.className = 'bank-img-viewer-thumb';
      var img = document.createElement('img');
      img.src = src;
      img.alt = name + ' 圖片 ' + (i + 1);
      img.addEventListener('click', function() {
        openFullscreen(images, i, name);
      });
      thumb.appendChild(img);
      imgViewerGrid.appendChild(thumb);
    });
  ...
  function closeBankImgViewer() {
    imgViewer.classList.remove('is-open');
    document.body.style.overflow = '';
  }

  function openFullscreen(imageList, index, bankName) {
    fsCurrentList = imageList;
    fsCurrentIndex = index;
    renderFullscreenImage(bankName);
    imgFullscreen.classList.add('is-open');
  }

  function renderFullscreenImage(bankName) {
    var total = fsCurrentList.length;
    var src = fsCurrentList[fsCurrentIndex];
    imgFsImg.src = src;
    imgFsImg.alt = (bankName || '') + ' 圖片 ' + (fsCurrentIndex + 1);
    if (imgFsCounter) imgFsCounter.textContent = (fsCurrentIndex + 1) + ' / ' + total;
    if (imgFsPrev) imgFsPrev.disabled = (fsCurrentIndex === 0);
    if (imgFsNext) imgFsNext.disabled = (fsCurrentIndex === total - 1);
  }

  function closeFullscreen() {
    imgFullscreen.classList.remove('is-open');
    fsCurrentList = [];
    fsCurrentIndex = 0;
  }

  if (imgViewerClose) imgViewerClose.addEventListener('click', closeBankImgViewer);
  if (imgViewerBackdrop) imgViewerBackdrop.addEventListener('click', closeBankImgViewer);
  if (imgFsClose) imgFsClose.addEventListener('click', closeFullscreen);
  if (imgFsBackdrop) imgFsBackdrop.addEventListener('click', closeFullscreen);

  if (imgFsPrev) {
    imgFsPrev.addEventListener('click', function() {
      if (fsCurrentIndex > 0) {
        fsCurrentIndex--;
  ...
  if (imgFsNext) {
    imgFsNext.addEventListener('click', function() {
      if (fsCurrentIndex < fsCurrentList.length - 1) {
        fsCurrentIndex++;
        renderFullscreenImage();
      }
    });
  ...
  var viewImgBtns = document.querySelectorAll('.btn-view-images');
  viewImgBtns.forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      e.stopPropagation();
      openBankImgViewer(btn.getAttribute('data-bank'));
    });
  });
